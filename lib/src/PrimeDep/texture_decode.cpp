...
uint32_t GXGetTexBufferSize(uint16_t width, uint16_t height, uint32_t fmt, bool mips, uint8_t maxLod) {
  ...
  uint32_t bitSize = fmt == GX_TF_RGBA8 || fmt == GX_TF_Z24X8 ? 64 : 32;
  uint32_t bufLen = 0;
  if (mips) {
    while (maxLod != 0) {
      ...
      for (uint32_t bx = 0; bx < bwidth; ++bx) {
        ...
        for (uint32_t y = 0; y < 2; ++y) {
          ...
          for (size_t x = 0; x < 2; ++x) {
            target[x].color1 = SBig(in[x].color1);
            target[x].color2 = SBig(in[x].color2);
      ...
      for (uint32_t xx = 0; xx < w; xx += 8) {
        for (uint32_t yb = 0; yb < 8; yb += 4) {
          for (uint32_t xb = 0; xb < 8; xb += 4) {
            ...
            const uint16_t color1 = SBig(*reinterpret_cast<const uint16_t*>(src));
            const uint16_t color2 = SBig(*reinterpret_cast<const uint16_t*>(src + 2));
            src += 4;
