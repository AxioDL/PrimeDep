...
std::shared_ptr<PakFile32Big> PakFile32Big::load(const std::string_view path) {
  auto ret = std::make_shared<PakFile32Big>(path);
  if (!ret->loadHeader()) {
    return nullptr;
  }
...
bool PakFile32Big::hasResource(const ObjectTag32Big& tag) {
...
bool PakFile32Big::hasNamedResource(std::string_view name) {
  return std::ranges::find_if(m_namedResources, [&name](const auto& descriptor) {
           return descriptor.name() == name;
         }) != m_namedResources.end();
};

ResourceDescriptor32Big PakFile32Big::descriptorByName(std::string_view name) {
  const auto& nameDesc =
...
ResourceDescriptor32Big PakFile32Big::descriptorById(const ObjectTag32Big& tag) {
...
std::tuple<const char*, uint32_t> PakFile32Big::loadData(const ResourceDescriptor32Big& desc) {
  if (!desc) {
    return {nullptr, 0};
  }
